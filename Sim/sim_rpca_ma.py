...
def error_cal(L_APG, S_APG, L, S, nonzero_ids, dim):
    RMSE_L = np.linalg.norm(L_APG - L, ord="fro") / dim
    RMSE_S = np.linalg.norm(S_APG - S, ord="fro") / dim
    mape_L = ((L_APG - L) / L).reshape(-1)
    MAPE_L = np.sum(abs(mape_L)) / (dim * dim) * 100
    ...
    return MAPE_L, MAPE_S, RMSE_L, RMSE_S


def Synthetic_data1_generate(rep_time):
    ...
    for seed in range(rep_time):
        ...
        for i_ord, i in enumerate(sigma):
            ...
            for j_ord, j in enumerate(p):
                ...
                MAPE_L, MAPE_S, RMSE_L, RMSE_S = error_cal(L_APG, S_APG, L, S, nonzero_ids, dim)
                ...
                MAPE_L, MAPE_S, RMSE_L, RMSE_S = error_cal(L_ma, S_ma, L, S, nonzero_ids, dim)
                ...
                data_iter = np.concatenate((M, L, S, L_APG, S_APG, L_ma, S_ma), axis=0)
                data_list.append(data_iter)
    with open('synthetic_data1.npy', 'wb') as f:
        np.save(f, data_list)
...
def Synthetic_data2_generate(rep_time):
    # L represents low-rank matrix; S represents sparse matrix.
    # Gaussian standard deviation
    sigma = 100
    # Corruption rate
    ...
    for seed in range(rep_time):
        ...
        for i_ord, i in enumerate(dim):
            for j_ord, j in enumerate(rank):
                ...
                L_APG = APG.get_low_rank()
                S_APG = APG.get_sparse()
                MAPE_L, MAPE_S, RMSE_L, RMSE_S = error_cal(L_APG, S_APG, L, S, nonzero_ids, i)
                MAPE_L_rpca[seed][i_ord][j_ord] = MAPE_L
                MAPE_S_rpca[seed][i_ord][j_ord] = MAPE_S
                RMSE_L_rpca[seed][i_ord][j_ord] = RMSE_L
                RMSE_S_rpca[seed][i_ord][j_ord] = RMSE_S
                ...
                L_ma = M_ma.to_numpy()
                S_ma = (M_obs - M_ma).to_numpy()
                MAPE_L, MAPE_S, RMSE_L, RMSE_S = error_cal(L_ma, S_ma, L, S, nonzero_ids, i)
                MAPE_L_ma[seed][i_ord][j_ord] = MAPE_L
                MAPE_S_ma[seed][i_ord][j_ord] = MAPE_S
                RMSE_L_ma[seed][i_ord][j_ord] = RMSE_L
                RMSE_S_ma[seed][i_ord][j_ord] = RMSE_S
                data_iter = np.concatenate((M, L, S, L_APG, S_APG, L_ma, S_ma), axis=0)
                data_list.append(data_iter)
    with open('synthetic_data2.npy', 'wb') as f:
        np.save(f, data_list)
    MAPE_L_ave_rpca = MAPE_L_rpca.mean(axis=0)
    MAPE_S_ave_rpca = MAPE_S_rpca.mean(axis=0)
    RMSE_L_ave_rpca = RMSE_L_rpca.mean(axis=0)
...
if __name__ == '__main__':
    Synthetic_data1_generate(rep_time=10)
    Synthetic_data2_generate(rep_time=10)